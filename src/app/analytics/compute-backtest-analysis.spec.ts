...
describe('modular analytics', () => {
  ...
  it('computes risk metrics from daily returns and reconciles drawdown', () => {
    ...
    expect(analysis.summary.totalPnl).toBe(analysis.performance.dailyPnl.reduce((sum, day) => sum + day.value, 0));
  ...
  it('tracks drawdown duration and recovery from the cumulative series', () => {
    const rows = [
      { ...hourlyDataset.rows[0], position: { quantityMw: 1, direction: 'long' as const }, prices: { spread: -10 } },
      { ...hourlyDataset.rows[3], position: { quantityMw: 1, direction: 'long' as const }, prices: { spread: 20 } },
    ];
  ...
  it('returns no rolling points until a complete window is available', () => {
    const analysis = computeBacktestAnalysis(hourlyDataset, { ...DEFAULT_ANALYSIS_SETTINGS, rollingWindowDays: 30 });
  ...
  it('produces generic segment metrics that reconcile to the total P&L', () => {
    ...
    const segments = groupPerformance(enriched, row => ({ key: row.strategy.tag, label: row.strategy.tag }), DEFAULT_ANALYSIS_SETTINGS);
