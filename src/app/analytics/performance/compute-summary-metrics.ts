import type { AnalysisSettings, DailyPerformance, EnrichedBacktestRow, RiskMetrics, SummaryMetrics } from '../../models/backtest.models';
import { aggregateReturns, annualizationPeriods, computeSharpe } from '../risk/compute-risk-metrics';
...
export const computeSummaryMetrics = (
  ...
  const longPnl = rows.filter(row => row.position.direction === 'long').reduce((sum, row) => sum + selectedPnl(row, settings.pnlMode), 0);
  const shortPnl = rows.filter(row => row.position.direction === 'short').reduce((sum, row) => sum + selectedPnl(row, settings.pnlMode), 0);
  ...
  return {
    ...
    longFrequency: { count: rows.filter(row => row.position.direction === 'long').length, pct: rows.length === 0 ? 0 : rows.filter(row => row.position.direction === 'long').length / rows.length * 100 },
    shortFrequency: { count: rows.filter(row => row.position.direction === 'short').length, pct: rows.length === 0 ? 0 : rows.filter(row => row.position.direction === 'short').length / rows.length * 100 },
    coverage: rows.length === 0 ? 0 : activeRows.length / rows.length * 100,
    hitRate: activeRows.length === 0 ? 0 : activeRows.filter(row => selectedPnl(row, settings.pnlMode) > 0).length / activeRows.length * 100,
